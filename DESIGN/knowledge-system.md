...
状态系统的 Effect `inject_knowledge` 主动推送。比如天气变为"暴风雨"时, Effect 配置了 `injectKnowledge: ["weather_storm"]`, 系统直接把对应的知识条目 ID 推入本轮的注入列表。
...
| 系统 | 交互方式 |
|------|---------|
| 状态系统 | 状态 Effect 的 inject_knowledge 推送特定条目 ID 到注入列表 |
